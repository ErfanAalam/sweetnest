...
/**
 * Per-property availability editor. Blocks / unblocks dates for a single
 * property via /api/calendar. Used by both the admin Calendar page and the
 * manager portal — access is enforced server-side based on the bearer token.
 */
export default function PropertyCalendarEditor({ propertyId }: { propertyId: string }) {
  ...
  const fetchCalendar = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/calendar?propertyId=${encodeURIComponent(propertyId)}`);
  ...
  const handleDateClick = (dateStr: string) => {
    ...
    if (!isBlocked(dateStr)) setShowModal(true);
    else toggleDate(dateStr, true);
  };
    ...
    try {
      const res = await fetch('/api/calendar', {
        ...
        body: JSON.stringify({
          ...
          reason: makeAvailable ? null : blockReason || 'Blocked',
        }),
      });
      if (res.ok) {
        if (makeAvailable) {
        ...
        setMessage(data.error || 'Could not update that date');
        setTimeout(() => setMessage(''), 3000);
      }
    } catch { /* silent */ }
            ...
            {[...Array(daysInMonth)].map((_, i) => {
              ...
              return (
                <button
                  ...
                  disabled={past || isProcessing || loading}
                  className={[
          ...
          <div className="card-premium">
            ...
            <div className="space-y-2.5 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-red-600 font-medium text-xs">{blockedThisMonth}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-stone-400 text-xs">Available Days</span>
                <span className="text-green-600 font-medium text-xs">{daysInMonth - blockedThisMonth}</span>
              </div>
            </div>
          </div>
